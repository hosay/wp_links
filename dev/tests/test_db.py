...
def test_add_and_get_account(db):
    add_account(
        ...
        username="editor1",
        password="pass1",
        vpn_conf_path="/vpn/editor1.conf",
...
def test_get_fixable_links_only_high_confidence(db):
    add_page(db, wiki_title="Ejemplo", found_via="wp_report")
    page_id = get_pending_pages(db)[0]["id"]
    ...
    set_replacement_url(db, bl_ids[1], "http://b-new.com", confidence="low", source="manual")

    links = get_fixable_links(db)
    assert len(links) == 1
    assert links[0]["original_url"] == "http://a.com"
